...
async function searchOnlineEnhanced(word, localResult) {
    ...
    // 如果没有本地结果，先显示词根分析
    if (!localResult) {
        combinedData.roots = RootAnalyzer.analyze(word);
    ...
    try {
        const [englishResult, chineseResult] = await Promise.allSettled(promises);
        
        // 添加英文释义
        if (englishResult.status === 'fulfilled' && englishResult.value) {
            if (combinedData.gloss) {
                combinedData.gloss += ' | ' + englishResult.value.definitions.join(' | ');
            } else {
                combinedData.gloss = englishResult.value.definitions.join(' | ');
            }
        ...
        if (chineseResult.status === 'fulfilled' && chineseResult.value) {
            combinedData.gloss = `中文: ${chineseResult.value} | ` + (combinedData.gloss || '未找到英文释义');
...
async function getChineseTranslation(word) {
    ...
    const translation = commonWords[word.toLowerCase()];
    if (translation) {
        return translation;
    }
    
    // 如果内置词汇没有，返回null（避免网络请求失败）
    return null;
}
        ...
        <div class="word-card">
            <div class="word-header">
                <div>
                    ...
                    <div class="badges">
                        ${data.sources.map(source => `<span class="source-badge">${source}</span>`).join('')}
                    </div>
                </div>
